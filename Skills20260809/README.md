# CyberStrikeAI Skills 技能包 v3.0

自动化AI渗透测试框架（CyberStrikeAI）的Skills包 - **深度攻防专家版**。

By:Tas9er

## 概述

基于 Eino ADK 的 CyberStrikeAI 框架通过统一工具调用 `skill("name")` 按需加载技能定义。所有技能包放置于 `$SKILLS_ROOT` 目录（默认同级目录），子目录名即技能名。
...
| 安全自动化 | `security-automation/` | 3.0.0 | **v3.0**：16章，DevSecOps全流程（SAST/DAST/SCA/IaC）+ SOAR深度 + Agentic AI安全自动化 + OPA策略即代码 + CIS Benchmark自动化 + 与cyberstrike-eino-demo联动 |
...
| 技能名称 | 目录 | 描述 |
|---------|------|------|
| Eino集成示例 | `cyberstrike-eino-demo/` | 框架集成示例，包含checklist/脚本/参考文档/配置文件等完整结构 |
